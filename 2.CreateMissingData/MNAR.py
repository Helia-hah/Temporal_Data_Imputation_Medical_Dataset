...
def create_mnar_missingness(dataset, target_column, ratio):
    ...
    X_miss_selfmasked =  produce_NA(not_null_dataset_values, p_miss=desired_ratio, mecha="MNAR", opt="selfmasked")
    ...
    target_column_after_mnar = pd.DataFrame({
        'index': not_null_dataset_index,
        target_column: pd.Series(X_mnar_selfmasked[:, 0], name=target_column)
        })
    target_column_after_mnar = target_column_after_mnar.set_index('index', drop=True)
    ...
    target_column_after_mnar_reindexed = target_column_after_mnar.reindex(full_index)
    ...
    dataset_missing = dataset.join(target_column_after_mnar_reindexed, how='inner')
...
def main():
    """Main function to process dataset and introduce MNAR missingness in various columns."""
    ...
    # Iterate through columns and missing data ratios, and apply MNAR missingness
    for column, initials in all_columns.items():
        for ratio in missing_data_ratios:

            # Apply MNAR missingness based on the ratio
            if ratio == 0.01:
                dataset_with_mnar = create_mnar_missingness(test_1h.copy(), column, ratio)
            elif ratio == 0.05:
                dataset_with_mnar = create_mnar_missingness(dataset_with_mnar.copy(), column, 0.04)
            else:
                dataset_with_mnar = create_mnar_missingness(dataset_with_mnar.copy(), column, 0.05)

            output_file = f'1h_test_mnar_{initials}_{ratio}.csv'
            dataset_with_mnar.to_csv(output_file, index=False)
